...
`.`/`..`, control characters, backslashes, and symbolic links are rejected.
...
atomic rename after revalidating the destination. A failed write leaves the old
file intact. Store traversal uses directory descriptors and no-follow opens to
prevent a link swap from redirecting a credential write or read.

## Environment paths and child commands

`--env VARIABLE` adds a sorted, idempotent entry to `secrets/env.zsh`. Entries
export the credential file's absolute path, never its value. Source the optional
file from shell startup code:
...
and every environment registration. An absent store is a valid empty state and
is not created by the check. It prints names and statuses only. It does not read
values, change permissions, replace links, or rewrite registrations.
...
filesystem calls and `x/term` for hidden terminal input. There are no external
cryptographic runtime dependencies.
